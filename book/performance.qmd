...
- Use the smallest necessary numeric types (so if you have an integer between 0 and 100, use `pl.UInt8`, not `pl.Int64`). This likely won't speed things up but will use less memory.
...
Also, the data is too small so I've concatenated it to itself 20 times. We're not doing anything that will care about the duplication.
...
def parse_date_pl(col: pl.Expr) -> pl.Expr:
    return col.str.strptime(pl.Date, fmt="%b %d, %Y")
...
def feet_to_cm_pl(col: pl.Expr) -> pl.Expr:
    ...
    total_inches = (
        ...
        + feet_inches_split.struct.field("field_1").str.rstrip('"').cast(pl.UInt8, strict=False)
    )
...
%%timeit
new_cols_pl = ([
    pl.col("Club").str.strip().cast(pl.Categorical),
...
Polars gets along well with NumPy [ufuncs](https://numpy.org/doc/stable/reference/ufuncs.html), even in lazy mode (which is weird because NumPy has no lazy API). Let's see how this looks by calculating the [great-circle distance](http://www.johndcook.com/blog/python_longitude_latitude/) between a bunch of coordinates.
...
The Pandas version tends to be ~25% faster on my machine:
...
- You can still make Polars slow if you do silly things with it, but it's easier to do the right thing with Polars.
