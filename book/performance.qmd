...
- Use the smallest necessary numeric types (so if you have an integer between 0 and 255, use `pl.UInt8`, not `pl.Int64`). This likely won't speed things up but will use less memory.
...
Also, the data is too small so I've concatenated it to itself 20 times. We're not doing anything that will care about the duplication. Here's how the raw data looks:
...
The Pandas version tends to be 10-60% faster on my machine:
