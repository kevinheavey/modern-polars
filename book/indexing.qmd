...
Polars is not yet popular enough to attact the same crowd, but when it does it should not run into the same problem, as the only ways to add or overwrite columns in Polars is the `with_columns` method.
