...
It'll probably break one day too, so the output is saved in this book's repository under `data/flights.csv.zip`.
...
if not dest.exists():
    ...
    with ZipFile(dest) as zf:
        zf.extract(zf.filelist[0].filename, path="data/")
...
``` {python}
df_pl.select(["Dest", "Tail_Number"]).take(list(range(12, 16)))
```
