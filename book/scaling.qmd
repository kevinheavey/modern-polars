...
We can solve this using Polars streaming mode, using Dask's lazy dataframe or simply using Pandas to read the files one by one and keep a running total:
...
daily_pl = (
    ...
    .filter(
        ...
        .is_between(date(2011, 1, 1), date(2017, 1, 1))
    )
    .with_column(pl.col("TRANSACTION_AMT") / 1000)
    .collect()
)
ax = (
    daily_pl.select(
