...
((
    occupation_avg_pl
    .sort("TRANSACTION_AMT", descending=True)
    .plot.barh(x="OCCUPATION", y="TRANSACTION_AMT", color="k", xlabel="Donation Amount", title="Average Donation by Occupation", legend="Average donation")
) * VLine(total_avg_pl)).opts(opts.VLine(color="C1", linewidth=3))
```
...
(
    ...
    .plot.line(x="TRANSACTION_DT", y=["DEM", "REP"], color=["C0", "C3"], line_width=3, title="Daily Donations (30-D Moving Average)", xlabel="Date")
)
```
## Dask
``` {python}
