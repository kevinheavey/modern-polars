...
::: {.callout-note}
Polars has [its own built-in plotting with hvPlot](https://docs.pola.rs/user-guide/misc/visualization/#built-in-plotting-with-hvplot) but the time series plots are [not great](https://github.com/holoviz/holoviews/issues/5112). Since most of the plots in this book are time series plots, we'll just use `.to_pandas()` followed by `.plot()` function after doing all the data manipulation in Polars.
:::
