...
::: {.callout-tip}
Q: `.with_columns` is for adding new columns or overwriting existing ones. But `.select` can also do those things, so what's the difference? 
...
::: {.callout-note}
Polars works fine with Matplotlib and Plotly, but it doesn't work with Seaborn or have its own `.plot` method. The latter two are very convenient
...
(
    ...
    .sort("DepTime")
    # every missing hour with 0 so the plot looks better
...
```python
flights_pl = (
    df_pl.select(pl.col(["FlightDate", "Tail_Number", "DepTime", "DepDelay"]))
    .drop_nulls()
    .sort("DepTime")
...
One new thing here: [window function](https://pola-rs.github.io/polars-book/user-guide/dsl/window_functions.html).
When Pandas code looks like `.groupby("country")["population"].transform("sum")`, the equivalent Polars code will look like
